...
namespace QSharpExercises.Solutions.Lab7 {
    ...
    operation Exercise1 (
        ...
        use (inputQubits, outputQubits) = (Qubit[inputSize], Qubit[inputSize]);
        ApplyPauliFromBitString(PauliX, true, input, inputQubits);
        op(inputQubits, outputQubits);

        let results = MultiM(outputQubits);
    ...
    operation Challenge2 (input : Qubit[], output : Qubit[]) : Unit {
        // output[0] <= NOT input[0] XOR input[1] XOR input[2]
        X(output[0]);
        ...
        // output[2] <= (input[0] XNOR input[1]) AND (NOT input[2]);
        X(output[1]);
